...
class TestAddNodePolicy(unittest.TestCase):
    ...
    def test_update_cluster_limits(self):
        """Test updating cluster limits."""
        result = self.policy.management("update_cluster_limits", {
            "max_nodes_per_cluster": 20,
            "max_gpus_per_cluster": 40
        })
        self.assertEqual(result["status"], "success")
        self.assertEqual(self.policy.parameters["max_nodes_per_cluster"], 20)
        self.assertEqual(self.policy.parameters["max_gpus_per_cluster"], 40)
        # self.assertEqual(result["timestamp"], "2025-05-21 07:06:35")
        # self.assertEqual(result["user"], "srikanthcognitifAI")
