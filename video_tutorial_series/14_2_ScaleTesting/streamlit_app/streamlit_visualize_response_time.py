...
def fetch_log_entries(db_cfg, start_epoch, end_epoch, test_id=None):
    ...
    engine = create_engine(
        ...
        pool_timeout=db_cfg.get('pool_timeout', 120),
        connect_args={'connect_timeout': db_cfg.get('connect_timeout', 120)},
...
def main():
    ...
    if submit:
        # Clear the old cache to ensure hitting submit always fetches fresh data!
        fetch_log_entries_cached.clear()
        
        # Always get the latest values from widgets to lock them in session for this submit
    ...
    if st.session_state.get('data_submitted', False):
        ...
        sub_test_id = params['test_id']
        
        st.sidebar.write(f"Epoch range: {start_epoch} - {end_epoch}")
        df = fetch_log_entries_cached(db_cfg, start_epoch, end_epoch, sub_test_id)
        if df.empty:
        ...
        # --- Response Time Distribution (Histogram / Bell Curve) ---
        dist_col1, dist_col2 = st.columns([2, 1])
        with dist_col1:
            st.markdown("### Response Time Distribution (Bell Curve)")
            
        max_rt = float(df['response_time'].max())
        min_rt = float(df['response_time'].min())
        slider_min = min(0.0, min_rt)
        slider_max = max(max_rt, slider_min + 0.1)
        
        with dist_col2:
            max_rt_limit = st.slider(
                'Max Response Time Filter (s)',
                min_value=slider_min,
                max_value=slider_max,
                value=max_rt
            )

        df_hist = df[df['response_time'] <= max_rt_limit]

        fig_hist = go.Figure()
        fig_hist.add_trace(go.Histogram(x=df_hist['response_time'], nbinsx=50, name='Requests', marker_color='#3366CC'))
        ...
        # --- Overall Response Time Percentiles (S-Curve) ---
        # Calculate percentiles from 1 to 100 on ALL data point response times
        percentiles = list(range(1, 101))
        percentile_values = [df['response_time'].quantile(p/100.0) for p in percentiles]
        
        fig_percentiles = go.Figure()
        fig_percentiles.add_trace(go.Scatter(x=percentiles, y=percentile_values, mode='lines', name='Response Time', fill='tozeroy', line=dict(color='#FF5733')))
        fig_percentiles.update_layout(
            title='Overall Response Time Percentiles (All Data)',
            xaxis_title='Percentile (%) / Request Distribution',
            yaxis_title='Response Time (s)',
            hovermode='x unified',
            height=500,
            font=dict(size=16),
