...
const model3Hits = new Counter('hits_qwen_block_3');
const model4Hits = new Counter('hits_qwen_block_4');
...
export function setup() {
    ...
    const testId = "TESTID_FROM_FIRST_LLM_BENCH";
...
const TARGET_RPS_1 = 150;
const TARGET_RPS_2 = 100;
const TARGET_RPS_3 = 50;
export const options = {
    scenarios: {
        inference_load: {
            executor: 'ramping-arrival-rate',
            startRate: TARGET_RPS_1,
            timeUnit: '1s',
            preAllocatedVUs: 500,
            maxVUs: TOTAL_SIMULATED_USERS,
            stages: [
                { duration: '1380m', target: TARGET_RPS_1 }, // Stay at RPS_1 for 10 mins
                { duration: '0s', target: TARGET_RPS_2 }, // Switch to RPS_2 immediately
                { duration: '30m', target: TARGET_RPS_2 }, // Stay at RPS_2 for 10 mins
                { duration: '0s', target: TARGET_RPS_3 }, // Switch to RPS_3 immediately
                { duration: '30m', target: TARGET_RPS_3 }, // Stay at RPS_3 for 10 mins
            ],
        },
    },
};
...
export default function (data) {
    ...
    const models = [
        "qwen3-5-0-8b-vllm-block-3",
        "qwen3-5-0-8b-vllm-block-4"
    ];
    ...
    else if (modelId === "qwen3-5-0-8b-vllm-block-3") model3Hits.add(1);
    else if (modelId === "qwen3-5-0-8b-vllm-block-4") model4Hits.add(1);
