...
class vLLMChatBlock:
    ...
    def __init__(self, context):
        ...
        self.batcher = Batcher(init_settings.get("batch_size", 4))
        self.mux_size = init_settings.get("mux_size", 1)
        self.muxer = Muxer(N=self.mux_size)
    ...
    def on_preprocess(self, packet):
        ...
        try:
            ...
            batch, is_timed_out, orig_size = self.batcher.add_to_batch({"packet": packet})
            if batch:
                ...
                for oneItemInBatch in batch:
                    ...
                    if isinstance(data, dict) and "inputs" in data:
                        ...
                        if len(data["inputs"]) > 1:
                            ...
                            #for item in data["inputs"]:
                            pass
                            #currently we shall retain any on gen_args
                            #club both string i.e message or prompt
                            any_one_gen_arg = {}
                            any_one_mode = ""
                            current_message = ""
                            system_message = self.blocks_system_message
                            for l,item in enumerate(data["inputs"]):
                                any_one_mode = item["mode"]
                                any_one_gen_arg: Dict[str, Any] = item.get("gen_params", item.get("generation_config", {}))
                                if "messages" in item and type(item["messages"]) is dict and "reply" in item["messages"]:
                                    pass
                                    current_message += f"{l}:" + item["messages"] + "\n"
                                    system_message: str = item.get("system_message", self.blocks_system_message)
                                elif "message" in item:
                                    pass
                                    current_message += f"{l}:" + item["message"] + "\n"
                                    system_message: str = item.get("system_message", self.blocks_system_message)
                                elif "prompt" in item:
                                    pass
                                    current_message += f"{l}:" + item["prompt"] + "\n"


                            session_id = packet.session_id
                            session_ids.append(session_id)
                            if any_one_mode=="chat":
                                message: str = current_message
                                #raise ValueError(f"Unsupported mode '{mode}' in input data")
                                if session_id not in self.chat_sessions:
                                    self.vllmUtil.create_chat_session(
                                        session_id,
                                        system_message=system_message
                                    )
                                    self.chat_sessions[session_id] = True
                                    print(f"Created new chat session: {session_id}")
                                if not self.vllmUtil.has_chat_session(session_id):
                                    self.vllmUtil.create_chat_session(
                                        session_id,
                                        system_message=system_message
                                    )
                                    ...
                                    print(f"Created new chat session: {session_id}")
                                #print(gen_params)
                                self.vllmUtil.add_message_to_chat(session_id, message)
                            prompts.append(current_message)
                            all_modes_of_batch.append(any_one_mode)
                            generation_args.append(self._merge_gen_args(any_one_gen_arg))
                            ...
                            if isinstance(item, dict):
                                if "messages" in item:
                                    ...
                                    if mode == "chat":
                                        ...
                                        #print(gen_params)
                                        self.vllmUtil.add_message_to_chat(session_id, message)
                                        self.vllmUtil.add_message_to_chat(session_id, system_message, "system")
                                        prompts.append(message)
                                    elif mode == "generate":
                                        #message: str = item.get("prompt", "hi")
                                        message: str = item.get("generated", item.get("prompt", "hi"))
                                        prompts.append(message)
                                    elif mode == "embed":
                    ...
                    #     #return True, [PreProcessResult(packet=packet, extra_data={"input": data}, session_id=packet.session_id)]
                        ...
                        mode = data["mode"]
                        session_id: str = packet.session_id
                        system_message: str = data.get("system_message", self.blocks_system_message)
                        # if mode == "chat" and not self.chat_supported:
                        #     raise RuntimeError("Loaded model does not support chat")
                        ...
                        if mode == "chat":
                            if "reply" in data:
                                data["message"] = data["reply"]
                                del data["reply"]
                            message: str = data.get("message", "hi")
                            #raise ValueError(f"Unsupported mode '{mode}' in input data")
                            if session_id not in self.chat_sessions:
                                self.vllmUtil.create_chat_session(
                                    session_id,
                            ...
                            message: str = data.get("generated", data.get("prompt", "hi"))
                            prompts.append(message)
                        elif mode == "embed":
            ...
            if len(all_modes_of_batch)>1:
                ...
                results = self.vllmUtil.generate_text(prompts, copy.deepcopy(generation_args))
                key = "generated"
                if results:
                    returnData = []
                    for i,session_id in enumerate(session_ids):
                        if is_timed_out and (i < orig_size): #return only true batch content
                    ...
                    for i,session_id in enumerate(session_ids):
                        if is_timed_out and (i < orig_size): #return only true batch content
                            ...
                            returnData.append(PreProcessResult(packet=batch[i]["packet"], extra_data={"input": {"mode":"generate","session_id":session_id,"gen_params":generation_args[i],"prompt":prompts[i],key:""}}, session_id=session_id))
                    return True, returnData
            elif "chat" in all_modes_of_batch:
                results = self.vllmUtil.run_chat_inference(session_ids, copy.deepcopy(generation_args))
                key = "reply"
                if results:
                    returnData = []
                    for i,session_id in enumerate(session_ids):
                        returnData.append(PreProcessResult(packet=batch[i]["packet"], extra_data={"input": {"mode":"chat","session_id":session_id,"gen_params":generation_args[i],"prompt":prompts[i],key:results[i]}}, session_id=session_id))
                    return True, returnData
                else:
                    returnData = []
                    for i,session_id in enumerate(session_ids):
                        returnData.append(PreProcessResult(packet=batch[i]["packet"], extra_data={"input": {"mode":"chat","session_id":session_id,"gen_params":generation_args[i],"prompt":prompts[i],key:""}}, session_id=session_id))
                    return True, returnData
            else:
    ...
    def on_data(self, preprocessed_entry, is_ws):
        ...
        try:
            ...
            if mode == "generate":
                ...
                response_text = input_data.get(key, "")
                #logger.debug(f"[on_data] mode={mode}, session_id={session_id}, gen_args={gen_params}, prompt={prompt}, {key}={response_text}")
            return True, OnDataResult(output={key: response_text, "mode": mode, "session_id": session_id,"gen_params":gen_params})
    ...
    def management(self, action: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if action == "save":
                ...
                ok = self.vllmUtil.save_model(path)
            ...
            if action == "tokenize":
                text = data.get("text", "")
                return {"tokens": self.vllmUtil.tokenize(text)}

            if action == "detokenize":
                tokens = data.get("tokens", [])
                return {"text": self.vllmUtil.detokenize(tokens)}
