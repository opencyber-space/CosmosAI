...
logger = logging.getLogger("AIOSMetrics")
...
class AIOSMetrics:
    ...
    def _reconnect_redis(self, *, start_backoff=1, max_backoff=30):
        host = os.getenv("METRICS_REDIS_HOST", "localhost")
        port = int(os.getenv("METRICS_REDIS_PORT", "6379"))  # allow env override
        backoff = start_backoff
        while not self.stop_event.is_set():
            try:
                # Create a fresh client with timeouts and retry_on_timeout
                self.redis_client = redis.StrictRedis(
                    host=host,
                    port=port,
                    db=0,
                ...
                logger.info("Redis reconnected at %s:%s", host, port)
                return
            except Exception as e:
                logger.warning(
                    "Redis reconnect failed: %s. Retrying in %ss (up to %ss).",
                    e, backoff, max_backoff
                )
                time.sleep(backoff)
    ...
    def write_to_redis(self):
        def _write_metrics():
            # Try an initial ping; if it fails, reconnect before entering steady loop
            try:
                self.redis_client.ping()
            except Exception:
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    # Collect Prometheus metric samples
                    for name, metric in self.metrics.items():
                        try:
                            ...
                            # Don't let one metric break the loop
                            logger.exception("Failed collecting samples for metric '%s'", name)

                    # Hardware and rolling summaries
                    try:
                        metrics_data["hardware"] = self.block_hardware_metrics.get_metrics()
                    except Exception:
                        logger.exception("Failed collecting hardware metrics")

                    try:
                        metrics_data.update(self.get_extended_metrics())
                    except Exception:
                        logger.exception("Failed computing extended/rolling metrics")

                    json_data = json.dumps(metrics_data)

                    # Ensure connection is alive; if not, reconnect
                    try:
                        self.redis_client.ping()
                    except Exception:
                    ...
                    # Push and log success
                    self.redis_client.lpush("NODE_METRICS", json_data)
                    logger.info("metrics pushed %s", datetime.datetime.utcnow().isoformat() + "Z")

                    # On success, reset loop backoff and sleep normal interval
                    loop_backoff = 1
                    time.sleep(30)

                except Exception as e:
                    # Catch-all so the loop never dies
                    logger.exception(f"Unexpected error in metrics writer loop: {e}")
                    time.sleep(loop_backoff)
                    loop_backoff = min(max_loop_backoff, loop_backoff * 2)

        write_thread = threading.Thread(target=_write_metrics, daemon=True)
        write_thread.start()
