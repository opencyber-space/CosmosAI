...
def build_history(messages: List[Dict[str, str]], history: int) -> List[Dict[str, str]]:
    """
    Build a messages list with:
      - First item always the fixed system message
      - Then the last `history` exchanges ending with the final user message.
    
    An 'exchange' is defined as:
      - The final user message alone (most recent)
      - Going backward before that: assistant + user pairs (if present)
    
    Assumptions:
      - The last item in `messages` is always a user message.
      - Messages are in chronological order.
    """
    SYSTEM_TEMPLATE = messages[0]

    # Ensure last message is user
    if messages[-1].get("role") != "user":
        raise ValueError("The last message must be a user message.")

    # Strip any leading system messages in the input; we will enforce our own
    start_idx = 0
    while start_idx < len(messages) and messages[start_idx].get("role") == "system":
        start_idx += 1
    core = messages[start_idx:]  # core dialog without initial system(s)

    if not core:
        return [SYSTEM_TEMPLATE.copy()]

    # We will collect exchanges from the end, ensuring we end at the final user.
    i = len(core) - 1
    collected: List[Dict[str, str]] = []

    # Always include the final user message (this is the first exchange).
    collected.append(core[i])
    i -= 1
    taken_exchanges = 1

    # For each additional exchange, take an assistant + user pair backwards if available.
    while taken_exchanges < history and i >= 0:
        # Expect an assistant before a user to form a pair; but tolerate missing assistant.
        # Step 1: try to take assistant (if present)
        if i >= 0 and core[i].get("role") == "assistance":
            collected.append(core[i])
            i -= 1
        # Step 2: then require a user before that to complete the pair
        if i >= 0 and core[i].get("role") == "user":
            collected.append(core[i])
            i -= 1
        else:
            # If there's no preceding user, stop pairing
            break
        taken_exchanges += 1

    # We collected from newest to oldest; reverse to chronological
    collected.reverse()

    # Final list: enforced system message + collected dialog
    result = [SYSTEM_TEMPLATE.copy()] + collected
    return result
...
def get_valid_tp(model_dir, num_gpus):
    ...
    with open(cfg_path) as f:
        cfg = json.load(f)
    num_heads = cfg.get("num_attention_heads") or cfg.get("num_key_value_heads")
    logger.info(f"num_attention_heads from config.json: {num_heads}")
    # Some architectures wrap these inside a 'model_type' specific dict or use 'n_heads'
    if num_heads is None:
        num_heads = cfg.get("n_head") or cfg.get("num_heads")
        logger.info(f"n_head/num_heads from config.json: {num_heads}")

    if num_heads is None:
        # 1. Try to get it from text_config (Modern Qwen/VLM models)
        text_cfg = cfg.get("text_config", {})
        num_heads = text_cfg.get("num_attention_heads")
        logger.info(f"num_attention_heads from text_config: {num_heads}")   
...
def get_best_fp8_dtype() -> str:
    ...
    sm_to_dtype = {
        ...
        100: "fp8_e5m2",  # Blackwell (B200, GB200) - Native FP4/FP6/FP8 support
    }
...
def estimate_kv_cache_bytes(model_config_path, max_seq_len, max_num_seqs, tp_size, dtype_size=1):
    ...
    if num_layers is None:
        ...
        logger.info(f"num_hidden_layers from text_config: {num_layers}")

    if hidden_size is None:
        # 1. Try to get it from text_config (Modern Qwen/VLM models)
        text_cfg = cfg.get("text_config", {})
        hidden_size = text_cfg.get("hidden_size")
        logger.info(f"hidden_size from text_config: {hidden_size}")
...
# def estimate_kv_cache_bytes2(model_config_path, max_seq_len, max_num_seqs, tp_size, dtype_size=1):
#     with open(os.path.join(model_config_path, "config.json")) as f:
#         cfg = json.load(f)
...
#     num_layers = text_cfg.get("num_hidden_layers") or cfg["num_hidden_layers"]
#     # For KV Cache, we need head_dim * num_kv_heads, NOT hidden_size
#     num_kv_heads = text_cfg.get("num_key_value_heads") or cfg["num_key_value_heads"]
#     head_dim = text_cfg.get("head_dim") or cfg["head_dim"]
    
#     # If head_dim isn't in config, it's usually hidden_size / num_attention_heads
#     if head_dim is None:
#         hidden_size = text_cfg.get("hidden_size") or cfg["hidden_size"]
#         num_heads = text_cfg.get("num_attention_heads") or cfg["num_attention_heads"]
#         head_dim = hidden_size // num_heads

#     # The KV Cache per layer is: 2 * (num_kv_heads * head_dim)
#     # Note: vLLM shards KV heads across TP. 
#     # If TP > num_kv_heads, each GPU still takes at least 1 head's worth of memory.
#     effective_kv_heads = max(1, num_kv_heads // tp_size) if num_kv_heads else 1
    
#     kv_per_layer_per_token = 2 * effective_kv_heads * head_dim * dtype_size
#     total_kv_per_gpu = kv_per_layer_per_token * num_layers * max_seq_len * max_num_seqs
    
#     return total_kv_per_gpu

def estimate_kv_cache_bytes2(model_config_path, max_seq_len, max_num_seqs, tp_size, dtype_size=2):
    with open(os.path.join(model_config_path, "config.json")) as f:
        cfg = json.load(f)
    
    # 1. Config Extraction (Handling nested text_config vs flat config)
    text_cfg = cfg.get("text_config", cfg)
    vision_cfg = cfg.get("vision_config", None)
    ...
    # Estimate Text Params (Embeddings + FFN + Attention/Linear Layers)
    # MLP: 3 * hidden * intermediate (Gate, Up, Down)
    # Attn/Linear: Roughly 4 * hidden^2
    text_params = (vocab_size * hidden_size) + \
                  (num_layers * (3 * hidden_size * intermediate_size + 4 * hidden_size**2))
    
    # Vision Weights (if present)
    vision_params = 0
    if vision_cfg:
        v_hidden = vision_cfg.get("hidden_size", 0)
        v_layers = vision_cfg.get("depth", 0) # Qwen uses 'depth'
        v_inter = vision_cfg.get("intermediate_size", 0)
        vision_params = v_layers * (v_hidden * v_inter * 2 + 4 * v_hidden**2)
        # Add Projector (out_hidden_size maps back to text hidden_size)
        vision_params += (v_hidden * text_cfg.get("hidden_size", 0))

    total_weights_bytes = (text_params + vision_params) * dtype_size * 1.10 # 10% overhead

    # --- HYBRID KV CACHE CALCULATION ---
    # Qwen 3.5 only uses standard KV cache for 'full_attention' layers
    layer_types = text_cfg.get("layer_types", ["full_attention"] * num_layers)
    ...
    effective_kv_heads = max(1, num_kv_heads // tp_size)
    
    kv_bytes_full = num_full_attn_layers * (2 * effective_kv_heads * head_dim * dtype_size) * max_seq_len * max_num_seqs

    # Fixed State Cache (Linear Attention / Gated DeltaNet)
    # Linear attention uses a fixed 'state' per sequence regardless of length.
    # Estimated at: state_dim * hidden_size * dtype
    linear_state_size = 128 # Typical for DeltaNet/Mamba
    kv_bytes_linear = num_linear_layers * (linear_state_size * hidden_size * dtype_size) * max_num_seqs

    total_kv_bytes = kv_bytes_full + kv_bytes_linear

    # --- WORKSPACE & CUDA GRAPHS ---
    total_gpu_mem_bytes = torch.cuda.get_device_properties(0).total_memory
    
    # Workspace scales with max_seq_len and multimodal overhead
    activation_size = max_num_seqs * max_seq_len * hidden_size * dtype_size * 6
    cuda_graph_buffer = 1.5 * (1024**3) # Static 1.5GB for graph captures
    image_buffer = 1.0 * (1024**3) if vision_cfg else 0
    
    total_needed = (total_weights_bytes + total_kv_bytes + activation_size + cuda_graph_buffer + image_buffer) * 1.15
    
    gpu_memory_utilization = round(total_needed / total_gpu_mem_bytes, 2)
    #keep 1GB of total_kv_bytes for safety
    return int(max(total_kv_bytes, 1 * 1024 * 1024 * 1024)), max(min(gpu_memory_utilization, 0.95), 0.1)
...
class vLLMUtils:
    def __init__(self, model_path, use_gpu=False, gpu_id=0, metrics=None, model_config={}, use_native_timings=True, cleanup_config=None):
        ...
        if "enable_thinking" in self.model_config:
            self.enable_thinking = self.model_config["enable_thinking"]
            del self.model_config["enable_thinking"]
        else:
        ...
        try:
            ...
            if "kv_cache_memory_bytes" not in self.model_config:
                if not cache_dtype:
                    cache_dtype = 2
                kvcache_bytes,gpu_memory_utilization = estimate_kv_cache_bytes2(
                        self.model_path,
                ...
                logger.info(f"Estimated KV Cache Memory: {kvcache_bytes} bytes")
                logger.info(f"Estimated KV Cache Memory: {kvcache_bytes/1024/1024/1024} GB")
                logger.info(f"Estimated GPU Memory Utilization: {gpu_memory_utilization}")
                self.model_config["kv_cache_memory_bytes"] = kvcache_bytes
                if "gpu_memory_utilization" not in self.model_config:
                    self.model_config["gpu_memory_utilization"] = gpu_memory_utilization
    ...
    def load_model(self):
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                self.metrics.set_instance_creation_time()
                self.model = LLM(
                ...
                def force_fp8_dtype(llm: LLM, dtype: str = "fp8e4b15"):
                    """
                    Force a specific fp8 dtype in the engine.
                    Compatible with both vLLM V1 (New) and Legacy engines.
                    """
                    try:
                        engine = llm.llm_engine
                        cache_cfg = None

                        # 1. Try Legacy/V2 location (Standard)
                        if hasattr(engine, "cache_config"):
                            cache_cfg = engine.cache_config
                        # 2. Try V1 location (Nested in vllm_config)
                        elif hasattr(engine, "vllm_config"):
                            cache_cfg = engine.vllm_config.cache_config
                        # 3. Try model_config fallback
                        elif hasattr(engine, "model_config"):
                            cache_cfg = getattr(engine.model_config, "cache_config", None)

                        if cache_cfg:
                            cache_cfg.cache_dtype = dtype
                            logger.info(f"⚡ Forced KV cache dtype to {dtype} in engine config")
                        else:
                            logger.warning("Could not find cache_config attribute in LLMEngine")

                        # Patch the Worker (Model Executor)
                        # In V1, the executor structure is significantly different
                        try:
                            executor = getattr(engine, "model_executor", None)
                            if executor:
                                # Check driver_worker (Legacy)
                                if hasattr(executor, "driver_worker"):
                                    worker = executor.driver_worker
                                    if hasattr(worker, "cache_config"):
                                        worker.cache_config.cache_dtype = dtype
                                # Check V1 Workers
                            ...
                            logger.debug(f"Worker-level FP8 patch skipped: {e}")

                    except Exception as e:
                        logger.error(f"Failed to force FP8 dtype: {e}. Falling back to default.")
    ...
    def generate_batch(self, batch):
        ...
        try:
            ...
            logger.error(f"Error during generate_batch: {e}")
            if self.metrics:
                try:
                    self.metrics.increment_inference_errors()
                    logger.info("✅ generate_batch error logged")
                except Exception as metrics_error:
                    logger.error(f"❌ ERROR logging generate_batch error: {metrics_error}")
            return None
    ...
    def add_message_to_chat(self, session_id, message, role="user"):
        ...
        if role == "system" and len(self.chat_sessions[session_id]["messages"]) > 0:
            self.chat_sessions[session_id]["messages"][0]["content"] = message
        else:
    ...
    def _handle_context_of_chat(self, session_ids, generation_args):
        try:
            n_ctx = self.model_config.get("n_ctx", 4096)
            safe_margin = int(0.125 * n_ctx)
            max_tokens = n_ctx - safe_margin

            max_model_len = self.model_config.get("max_model_len",256)

            #print("before",session["messages"])
            """
            Always keep the system prompt (index 0)
            This would run tokenize first and then check length of each token, if theyare not less that max_tokens, it will remove the oldest user/assistant message (index 1)
            """
            # prompt_tokens_list = []
            # for i,session_id in enumerate(session_ids):
            #     gen_args = generation_args[i]
            #     max_tokens = gen_args.get("max_tokens", min(128,max_model_len))
            #     prompt_tokens = 0
            #     while True:
            ...
            """
            We can directly take param from user to set max history to keep
            If history = 1, then keep system and user message only
            If history = 2, then keep system, user, assistant, user messages only
            and so on...
            """
            final_prompt = []
            for i,session_id in enumerate(session_ids):
                ...
                gen_args = generation_args[i]
                max_history = gen_args.get("max_history", 1)
                if "max_history" in generation_args[i]:
                    #remove it as it is not a valid param for vLLM
                    del generation_args[i]["max_history"]
                #loop such that we will retain only max_history messages plus system prompt
                current_session_id_messages = build_history(session["messages"], history = max_history)
                final_prompt.append(current_session_id_messages)
            return final_prompt
        except Exception as e:
    ...
    def run_chat_inference(self, session_ids, generation_args):
        MAX_RETRIES = 3
        RETRY_DELAY = 5
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                print(f"[DEBUG] run_chat_inference called for session_id: {session_ids}")
                from vllm import SamplingParams
                import torch
                if not self.model:
                    logger.error("Model is not loaded.")
                ...
                for session_id in session_ids:
                    if session_id not in self.chat_sessions:
                        raise Exception(f"session_id {session_id} not found in chat_sessions: {list(self.chat_sessions.keys())}")
                ...
                conversations_list = self._handle_context_of_chat(session_ids,generation_args)

                num_sequences = len(session_ids)
                sampling_params = [SamplingParams(**generation_args[i]) for i in range(num_sequences)]
                ...
                total_time = end - start
                logger.info(f"duration of inference: {total_time}")

                resultsToReturn = []

                for i, output in enumerate(results):
                    logger.debug(f"=== Conversation {i+1} ===")
                    logger.debug(f"Prompt: {conversations_list[i]}")
                    logger.debug(f"Generated: {output.outputs[0].text}")
                    if len(output.outputs[0].text) <= 1:
                        logger.warning(f"⚠️ Generated text is very short (<=1 char), 🛑")
                        if self.metrics:
                    ...
                    if self.metrics:
                        ...
                        # print("[DEBUG] Updating rolling metrics for autoscaling...")
                        self.metrics.update_last_processed_time(time.time())
                        # print("[DEBUG] update_last_processed_time completed")
                ...
                # # Dump all metrics
                # for metric in self.model.get_metrics():
                ...
                logger.error(f"Error run_chat_inference [CUDA OOM] {e}")
                if attempt == MAX_RETRIES:
                    if self.metrics:
                        self.metrics.increment_inference_errors()
                        return None
            except torch.cuda.CudaError as e:
                logger.error(f"Error run_chat_inference[CUDA kernel error] {e}")
                if attempt == MAX_RETRIES:
                    if self.metrics:
                        self.metrics.increment_inference_errors()
                        return None
            except RuntimeError as e:
                logger.error(f"Error run_chat_inference [Runtime error] {e}")
                if attempt == MAX_RETRIES:
                    if self.metrics:
                        self.metrics.increment_inference_errors()
                        return None
            except Exception as e:
                logger.error(f"Error run_chat_inferencet [Unexpected {type(e).__name__}] {e}")
                if attempt == MAX_RETRIES:
                    if self.metrics:
                        self.metrics.increment_inference_errors()
                        return None
