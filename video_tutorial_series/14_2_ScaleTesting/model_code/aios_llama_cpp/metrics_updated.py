...
class LLMMetricsUpdated:
    ...
    def set_model_reference(self, model, use_native_timings=True):
        ...
        logger.info(f"📊 Model reference set for native timings (enabled: {use_native_timings})")

    def set_instance_creation_time(self):
        self.metrics.set_gauge("instance_creation_time", time.time())
    ...
    def _get_native_timings(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            if hasattr(self._model_reference, '_ctx') and hasattr(self._model_reference._ctx, 'get_timings'):
                ...
                logger.info(f"🔧 Native timings retrieved: {timings}")
                return timings
        except Exception as e:
            logger.warning(f"⚠️ Could not retrieve native timings: {e}")
    ...
    def _register_llm_metrics(self):
        ...
        self.metrics.register_counter("llm_inference_empty_token", "Total number of empty inference in batch")
        self.metrics.register_gauge("llm_last_processed_time", "Timestamp of the last processed inference")
    ...
    def log_prompt(self, prompt_token_count: int):
        ...
        logger.info(f"📊 log_prompt called with {prompt_token_count} tokens")
        try:
            self.metrics.increment_counter("llm_prompts_total")
            print("[DEBUG] llm_prompts_total incremented")
            logger.info("✅ llm_prompts_total incremented")
            ...
            if counter:
                ...
                logger.info(f"✅ llm_prompt_tokens_total incremented by {prompt_token_count}")
            else:
                print("[DEBUG] llm_prompt_tokens_total counter not found, registering...")
                logger.warning("⚠️ llm_prompt_tokens_total counter not found, registering...")
                ...
                if counter:
                    ...
                    logger.info(f"✅ llm_prompt_tokens_total registered and incremented by {prompt_token_count}")
                else:
                    print("[DEBUG] Failed to register llm_prompt_tokens_total counter")
                    logger.error("❌ Failed to register llm_prompt_tokens_total counter")
    ...
    def log_response(self, generated_token_count: int):
        ...
        logger.info(f"📊 log_response called with {generated_token_count} tokens")
        try:
            # Fix: Use the correct API for incrementing by a specific value
            ...
            if counter:
                ...
                logger.info(f"✅ llm_tokens_generated_total incremented by {generated_token_count}")
            else:
                print("[DEBUG] llm_tokens_generated_total counter not found, registering...")
                logger.warning("⚠️ llm_tokens_generated_total counter not found, registering...")
                # Counter doesn't exist, register it first
                self.metrics.register_counter("llm_tokens_generated_total", "Total number of tokens generated by the LLM")
                counter = self.metrics.metrics.get("llm_tokens_generated_total")
                if counter:
                    ...
                    logger.info(f"✅ llm_tokens_generated_total registered and incremented by {generated_token_count}")
                else:
                    print("[DEBUG] Failed to register llm_tokens_generated_total counter")
                    logger.error("❌ Failed to register llm_tokens_generated_total counter")
            ...
            logger.error(f"❌ ERROR in log_response: {e}")
            import traceback
            print(traceback.format_exc())
            logger.error(f"Traceback: {traceback.format_exc()}")
    ...
    def observe_inference_time(self, start_time: float):
        ...
        logger.info(f"📊 observe_inference_time called with calculated elapsed: {calculated_elapsed:.4f}s")
        ...
        if native_timings and self._use_native_timings:
            ...
            if 'eval' in native_timings:
                ...
                if native_elapsed > 0:
                    ...
                    logger.info(f"🔧 Using native inference timing: {final_elapsed:.4f}s (vs calculated: {calculated_elapsed:.4f}s)")
                ...
                if total_ms > 0:
                    ...
                    logger.info(f"🔧 Using native total timing: {final_elapsed:.4f}s")
        
        try:
            self.metrics.observe_histogram("llm_inference_duration_seconds", final_elapsed)
            logger.info(f"✅ llm_inference_duration_seconds histogram updated with {final_elapsed:.4f}s")
            ...
            logger.info(f"✅ llm_inference_duration_rolling updated with {final_elapsed:.4f}s")
        except Exception as e:
            logger.error(f"❌ ERROR in observe_inference_time: {e}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
    ...
    def observe_time_to_first_token(self, start_time: float):
        ...
        #             logger.info(f"🔧 Using native TTFT: {final_ttft:.4f}s")
    ...
    def observe_resource_utilization(self, cpu_util: float, gpu_util: float, mem_bytes: float):
        ...
        try:
            ...
            logger.info(f"✅ Resource utilization gauges updated: CPU {cpu_util}%, GPU {gpu_util}%, Mem {mem_bytes}B")
        except Exception as e:
            logger.error(f"❌ ERROR in observe_resource_utilization: {e}")
    ...
    def observe_time_per_output_token(self, start_time: float, token_count: int):
        ...
        Will use native llama_cpp timings if available and enabled.
        """
        logger.info(f"📊 observe_time_per_output_token called with token_count: {token_count}")
        try:
            if token_count > 0:
                ...
                if native_timings and self._use_native_timings:
                    ...
                    if 'eval' in native_timings:
                        ...
                        if eval_time_ms > 0 and eval_tokens > 0:
                            ...
                            logger.info(f"🔧 Using native TPOT: {final_tpot:.4f}s per token")
                
                logger.info(f"📊 Final TPOT: {final_tpot:.4f}s per token")
                self.metrics.observe_histogram("llm_time_per_output_token_seconds", final_tpot)
                logger.info(f"✅ llm_time_per_output_token_seconds histogram updated")
                
                # Add to rolling metrics for autoscaling (Performance)
                self.metrics.observe_rolling("llm_tpot_rolling", final_tpot)
                logger.info(f"✅ llm_tpot_rolling updated with {final_tpot:.4f}s")
            else:
                logger.warning("⚠️ Token count is 0, skipping TPOT calculation")
        except Exception as e:
            logger.error(f"❌ ERROR in observe_time_per_output_token: {e}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
    ...
    def update_tokens_per_second(self, tokens_generated: int, duration_seconds: float):
        ...
        Will use native llama_cpp timings if available and enabled.
        """
        logger.info(f"📊 update_tokens_per_second called with tokens: {tokens_generated}, duration: {duration_seconds:.4f}s")
        try:
            ...
            if native_timings and self._use_native_timings:
                if 'eval' in native_timings:
                    ...
                    if eval_time_ms > 0 and eval_tokens > 0:
                        ...
                        logger.info(f"🔧 Using native timing data: {final_tokens} tokens in {final_duration:.4f}s")
            
            if final_duration > 0:
                tps = final_tokens / final_duration
                logger.info(f"📊 Calculated TPS: {tps:.2f} tokens/sec")
                self.metrics.set_gauge("llm_tokens_per_second", tps)
                logger.info(f"✅ llm_tokens_per_second gauge set to {tps:.2f}")
                
                # Add to rolling metrics for autoscaling (Performance)
                self.metrics.observe_rolling("llm_tps_rolling", tps)
                logger.info(f"✅ llm_tps_rolling updated with {tps:.2f}")
            else:
                logger.warning("⚠️ Duration is 0, skipping TPS calculation")
        except Exception as e:
            logger.error(f"❌ ERROR in update_tokens_per_second: {e}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")

    def update_last_processed_time(self, update_time: float):
        """Update last processed time"""
        self.metrics.set_gauge("llm_last_processed_time", update_time)

    def increment_inference_empty_tokens(self):
        """Increment empty inference tokens counter"""
        self.metrics.increment_counter("llm_inference_empty_token")
