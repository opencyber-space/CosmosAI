...
class AIOSv1PolicyRule:
    def __init__(self, rule_id, settings, parameters):
        ...
        self.min_age_to_downscale = self.parameters.get("min_age_to_downscale", 300) # New: 5 minutes default
        self.scale_down_needed = self.parameters.get("scale_down_needed", False)
    ...
    def _get_queue_length(self, instance: Dict[str, Any]) -> float:
        ...
        if isinstance(queue_length_data, dict):
            ...
            return queue_length_data.get("average_5m", 0)
    ...
    def eval(self, parameters: Dict[str, Any], input_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            if instance_count > self.min_replicas and self.scale_down_needed:
                ...
                candidate_creation_time = candidate.get("instance_creation_time", time.time())
                ...
                if avg_queue_length < self.queue_down_threshold:
                    ...
                    if (time.time()-candidate_creation_time) < self.min_age_to_downscale:
                        self.logger.info(f"Downscale skipped(avg queue check): candidate {candidate_id} is too young.")
                        return {"skip": True, "reason": "Downscale skipped: candidate is too young."}
                ...
                if last_activity_timestamp > 0 and idle_duration > self.idle_time_downscale_threshold:
                    ...
                    if (time.time()-candidate_creation_time) < self.min_age_to_downscale:
                        self.logger.info(f"Downscale skipped(idleness check): candidate {candidate_id} is too young.")
                        return {"skip": True, "reason": "Downscale skipped: candidate is too young."}
