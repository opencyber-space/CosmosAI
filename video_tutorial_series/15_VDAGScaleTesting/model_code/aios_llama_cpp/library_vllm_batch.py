...
class vLLMUtils:
    def __init__(self, model_path, use_gpu=False, gpu_id=0, metrics=None, model_config={}, use_native_timings=True, cleanup_config=None):
        ...
        try:
            ...
            #check if fp8 present and HW supports this
            # A100 doesnt support fp8 
            # L4 supports fp8
            # if "fp8" in self.model_config.get("kv_cache_dtype", "fp8"):
            #     if fp8Support():
            #         logger.info("Using fp8 kv_cache_dtype")
            #     else:
            #         logger.warning("FP8 not supported on this GPU, falling back to bf16")
            #         logger.warning("Ignore above message as FP8 works with V0 engine in L4 and A100")
            #         #Commennted this as i checked fp8 worked with v0 engine in L4 and A100
            #         #if we keep auto it chooses v1 but doent provides metrics per sequence
            #         #self.model_config["kv_cache_dtype"] = "auto"

            # ####################NOT REALLY NEEDED NOW, AS WE ESTIMATE KV-CACHE BELOW
            # per_gpu_totals = _get_per_gpu_memory_bytes()
            # per_gpu_total = per_gpu_totals[0] if per_gpu_totals else 24 * 1024 * 1024 * 1024
            # per_gpu_free_list = _get_per_gpu_free_bytes()
            # per_gpu_free = per_gpu_free_list[0] if per_gpu_free_list else None

            # # Estimate model weight on disk (bytes)
            # model_size_bytes = 0
            # try:
            #     if os.path.exists(self.local_model_name):
            #         if os.path.isfile(self.local_model_name):
            #             model_size_bytes = os.path.getsize(self.local_model_name)
            #         else:
            #             # Sum sizes in directory
            #             for root, _, files in os.walk(self.local_model_name):
            #                 for fn in files:
            #                     try:
            #                         model_size_bytes += os.path.getsize(os.path.join(root, fn))
            #                     except Exception:
            #                         pass
            # except Exception:
            #     model_size_bytes = 0

            # # Heuristic: estimate in-memory weight per GPU (factor) and divide by tensor-parallel shards
            # weight_factor = 1.2  # weights in memory ~ file size * factor
            # est_weight_total = int(model_size_bytes * weight_factor) if model_size_bytes > 0 else int(8 * 1024 * 1024 * 1024)
            # est_weight_per_gpu = max(0, est_weight_total // self.model_config['tensor_parallel_size'])

            # # Reserve 1 GiB for headroom (user requirement)
            # reserve_bytes = int(1 * 1024 * 1024 * 1024)
            # # Small overhead estimates (activations, non-torch, CUDA graph)
            # overhead_bytes = int(512 * 1024 * 1024)  # 512MB

            # # Compute candidate kv-cache memory per GPU using current free memory (preferred)
            # if per_gpu_free is not None:
            #     # Use observed free memory on GPU (accounts for other processes)
            #     kv_cache_candidate = per_gpu_free - reserve_bytes - est_weight_per_gpu - overhead_bytes
            # else:
            #     # Fallback to total memory heuristic
            #     kv_cache_candidate = per_gpu_total - reserve_bytes - est_weight_per_gpu - overhead_bytes
            # if kv_cache_candidate < 0:
            #     kv_cache_candidate = 0

            # # Final kv cache in bytes (align to MiB)
            # kv_cache_mib = (kv_cache_candidate // (1024 * 1024))
            # kv_cache_bytes = int(kv_cache_mib * 1024 * 1024)

            # # # prefer key name 'kv_cache_memory' expected by vLLMUtils/init wrapper
            # # self.model_config['kv_cache_memory'] = kv_cache_bytes
            # # # remove gpu_memory_utilization to avoid conflicting directives
            # # if 'gpu_memory_utilization' in self.model_config:
            # #     del self.model_config['gpu_memory_utilization']

            # logger.info(f"Auto GPU detection: num_gpus={self.model_config['tensor_parallel_size']}, per_gpu_total={per_gpu_total/(1024**3):.2f} GiB")
            # logger.info(f"Estimated model weight total={est_weight_total/(1024**3):.2f} GiB, per GPU={est_weight_per_gpu/(1024**3):.2f} GiB")
            # logger.info(f"Computed kv_cache_memory={kv_cache_bytes} bytes ({kv_cache_mib} MiB) and tensor_parallel_size={self.model_config['tensor_parallel_size']}")
    ...
    def generate_text(self, prompts, generation_configs):
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                # def force_fp8_dtype(llm, dtype: str = "fp8e4b15"):
                #     """Force a specific fp8 dtype in the engine (bypassing pydantic validator)."""
                #     engine = llm.llm_engine
                #     cache_cfg = engine.cache_config
                #     cache_cfg.cache_dtype = dtype
