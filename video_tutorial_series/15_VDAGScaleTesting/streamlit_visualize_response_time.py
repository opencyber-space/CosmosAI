...
def get_db_config():
    with open('config_nonvdag.yaml', 'r') as f:
...
def fetch_log_entries(db_cfg, start_epoch, end_epoch, test_id=None):
    ...
    try:
        sql = f"""
            SELECT session_id, seq_no, type, response_time, raw, test_id, starttime, endtime, starttimeObj, endtimeObj
            FROM {table_name}
...
def main():
    ...
    if submit:
        ...
        # Clear cached results for fresh fetch when user clicks Submit
        try:
            fetch_log_entries_cached.clear()
        except Exception:
            # .clear may not be available in older Streamlit versions; ignore if it fails
            pass
        ...
        # --- Average Requests per window (using starttime) ---
        # Bin by avg_window seconds on the 'starttime' field and compute average requests per session in each window
        # This shows how many requests (on average) each session issued during each time window (based on starttime)
        df_start_sorted = df.sort_values('starttime')
        if df_start_sorted['starttime'].isnull().all():
            st.warning('No starttime data available to compute requests-per-window.')
        else:
            min_start = int(df_start_sorted['starttime'].min())
            max_start = int(df_start_sorted['starttime'].max())
            if min_start == max_start:
                # single timestamp: create one bin that includes it
                bins_start = [min_start, min_start + max(1, avg_window)]
            else:
                bins_start = list(range(min_start, max_start + avg_window, avg_window))
            df_start_sorted['bin_start'] = pd.cut(df_start_sorted['starttime'], bins=bins_start, labels=bins_start[:-1], include_lowest=True)
            # total requests and unique sessions per bin
            reqs_by_bin = df_start_sorted.groupby('bin_start').agg(
                total_requests=('session_id', 'size'),
                unique_sessions=('session_id', 'nunique')
            ).reset_index()
            # compute requests per window and per-second rate
            reqs_by_bin['requests_per_window'] = reqs_by_bin['total_requests']
            reqs_by_bin['requests_per_sec'] = reqs_by_bin['requests_per_window'] / float(max(1, avg_window))
            # convert bin label to datetime for plotting
            reqs_by_bin['bin_dt'] = reqs_by_bin['bin_start'].astype(float).apply(lambda x: datetime.fromtimestamp(x, ist_tz))
            fig_reqs = go.Figure()
            # primary: total requests in the window (this will be ~ avg_window * rps)
            fig_reqs.add_trace(go.Scatter(
                x=reqs_by_bin['bin_dt'],
                y=reqs_by_bin['requests_per_window'],
                mode='lines+markers',
                name=f'Requests per Window ({avg_window}s window, by starttime)'
            ))
            # secondary: requests per second (same data normalized by window length)
            fig_reqs.add_trace(go.Scatter(
                x=reqs_by_bin['bin_dt'],
                y=reqs_by_bin['requests_per_sec'],
                mode='lines',
                name='Requests per second',
                yaxis='y2'
            ))
            fig_reqs.update_layout(
                title=f'Requests per Window (Window: {avg_window} seconds, binned by starttime)',
                xaxis_title='Time (window start)',
                yaxis=dict(title='Requests per Window'),
                yaxis2=dict(title='Requests per second', overlaying='y', side='right'),
                hovermode='x unified',
                height=400,
                font=dict(size=16),
                margin=dict(l=60, r=60, t=60, b=60)
