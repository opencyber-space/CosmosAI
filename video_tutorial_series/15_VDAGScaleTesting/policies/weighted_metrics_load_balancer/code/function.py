...
class AIOSv1PolicyRule:
    def __init__(self, rule_id, settings, parameters):
        ...
        self.tie_breaker = self.parameters.get("tie_breaker", "first")  # Options: first, round_robin
        ...
        # New: running counter for requests since last metrics update
        self.instance_request_counters = {}
        ...
        try:
            ...
            if need_fetch:
                try:
                    ...
                    if metrics and "block_metrics" in metrics and metrics["block_metrics"] and len(metrics["block_metrics"]) > 1:
                        self.cached_metrics = metrics
                        self.logger.info(f"Fetched fresh metrics from metrics_collector { len(metrics['block_metrics'])}")
                    else:
                    ...
                    valid_block_metrics = [ inst_data.get('instanceId') for inst_data in  metrics.get("block_metrics", []) if inst_data.get('instanceId') != "executor" ]
                    self.last_metrics_time = time.time()
                    # Reset running counters on fresh metrics fetch
                    for inst in self.current_instances:
                        if inst in valid_block_metrics:
                            self.instance_request_counters[inst] = 0
                            self.logger.info(f"Reset running counter for instance {inst}")
            ...
            if set(latest_instances) != set(self.current_instances):
                ...
                # Update running counters for instance changes
                for inst in new_instances:
                    self.instance_request_counters[inst] = 0
                # Remove counters for removed instances
                removed_instances = [inst for inst in self.previous_instances if inst not in latest_instances]
                for inst in removed_instances:
                    self.instance_request_counters.pop(inst, None)
            ...
            if self.selection_mode == "equal":
                ...
                # Select based on least combined queue length + running counter for equal distribution
                instance_combined_scores = {}
                for inst in instances_with_metrics:
                    instance_id = inst.get('instanceId')
                    queue_length = self._get_metric_value(inst, 'queue_length', input_data)
                    running_counter = self._get_metric_value(inst, 'running_counter', input_data)
                    combined_score = queue_length + running_counter  # Equal weight for simplicity
                    instance_combined_scores[instance_id] = combined_score
                    self.logger.info(f"Individual scores (queue , running_counter) for selection of instance : {instance_id} {queue_length} {running_counter}")
                self.logger.info(f"Combined scores (queue + running_counter) for selection: {instance_combined_scores}")
                
                min_combined = min(instance_combined_scores.values())
                candidates = [inst_id for inst_id, cs in instance_combined_scores.items() if cs == min_combined]
                
                if len(candidates) > 1:
                    self.logger.info(f"Found {len(candidates)} instances with same combined score ({min_combined}), applying tie-breaking")
                ...
                self.logger.info(f"Selected instance {selected_instance} for equal distribution (combined score: {min_combined})")
            ...
            # Increment running counter for selected instance
            self.instance_request_counters[selected_instance] = self.instance_request_counters.get(selected_instance, 0) + 1
    ...
    def management(self, action: str, data: dict) -> dict:
        ...
        if action == "health_check":
            ...
            if chosen_instance:
                ...
                self.session_timestamps[session_id] = time.time()  # Record timestamp for timeout
                # Increment running counter for selected instance
                self.instance_request_counters[chosen_instance] = self.instance_request_counters.get(chosen_instance, 0) + 1
    ...
    def _redistribute_sessions(self, block_metrics=None):
            self.logger.info("Starting redistribution process")
            ...
            if block_metrics:
                ...
                if queue_lengths:
                    max_q = max(queue_lengths.values())
                    min_q = min(q for q in queue_lengths.values() if q != float('inf'))
                    if max_q != float('inf') and (max_q - min_q > self.imbalance_threshold):
                        imbalance_detected = True
                        avg_metric = self._calculate_average_queue(queue_lengths)
                        self.logger.info(f"Queue Imbalance ({max_q - min_q}) > threshold ({self.imbalance_threshold}), proceeding.")
                ...
                if task_counts_by_instance:
                    ...
                    if max_tasks - min_tasks > self.task_imbalance_threshold:
                        imbalance_detected = True
                        avg_metric = sum(task_counts_by_instance.values()) / len(task_counts_by_instance)
                        self.logger.info(f"Task Imbalance ({max_tasks - min_tasks}) > threshold ({self.task_imbalance_threshold}), proceeding.")

            if not imbalance_detected:
                self.logger.info("System is balanced, skipping redistribution.")
                return

            # Identify overloaded instances
            overloaded_instances = []
            if block_metrics:
                overloaded_instances = self._identify_overloaded_instances(queue_lengths, avg_metric)
                self.logger.info(f"Overloaded instances (above avg queue {avg_metric:.2f}): {overloaded_instances}")
                ...
                self.logger.info(f"Overloaded instances (above avg tasks {avg_metric:.2f}): {overloaded_instances}")

            # Collect sessions to redistribute
            sessions_to_redistribute = []
            if block_metrics:
                sessions_to_redistribute = self._collect_sessions_to_redistribute(overloaded_instances, sessions_by_instance, queue_lengths, avg_metric)
            ...
            self.logger.info(f"Identified {len(sessions_to_redistribute)} sessions to redistribute.")
            ...
            for session in sessions_to_redistribute:
                ...
                if not old_instance: continue

                available_targets = [i for i in self.current_instances if i != old_instance and i not in overloaded_instances]
                ...
                if self.redistribution_selection_mode == "least_loaded":
                    if block_metrics:
                        ...
                        target_metrics = {inst: task_counts_by_instance.get(inst, 0) for inst in available_targets}
                        new_instance = min(target_metrics, key=target_metrics.get)
                ...
                if new_instance and new_instance != old_instance:
                    ...
                    # Update local counts for accurate selection in the same cycle
                    ...
                    self.logger.debug(f"Redistributed session {session} from {old_instance} to {new_instance}")
            self.logger.info(f"Total sessions redistributed: {len(sessions_to_redistribute)}")
