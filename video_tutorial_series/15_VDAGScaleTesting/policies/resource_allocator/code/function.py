...
class AIOSv1PolicyRule:
    ...
    def _score_gpu_memory(self, available_nodes: List[Dict[str, Any]], 
        ...
        try:
            ...
            # 1. Prefer nodes/GPUs where blockID is already present
            preferred_gpus = find_gpus_with_blockid(
                available_nodes, block_id, self.max_model_per_gpu, self.min_threshold_gpu_mem_non_model
            )
            if preferred_gpus:
                logger.info(f"Found preferred GPUs with existing blockID {block_id}: {preferred_gpus}")
                for node, gpu in preferred_gpus:
                    if gpu.get("freeMem", 0) >= gpu_memory_required:
                        # Score this node/gpu highly (e.g., set total_score = 1.0)
                        # ...existing scoring logic...
                        logger.info(f"Preferentially selecting node {node['id']} GPU {gpu['gpu_id']} for blockID {block_id}")
                        # Return or prioritize this node/gpu
...
def find_gpus_with_blockid( nodes, block_id, max_model_per_gpu, min_threshold_gpu_mem_non_model):
    """
    Returns a list of (node, gpu) tuples where block_id is already present and model count < max_model_per_gpu.
    """
    result = []
    for node in nodes:
        for gpu in node.get('gpu_metrics_individual', []):
            # Count models
            model_count = sum(
                1 for inst in gpu.get('instances', [])
                if inst.get('totalUsedMem', 0) > min_threshold_gpu_mem_non_model
            )
            # Check if block_id is present
            blockid_present = any(
                inst.get('blockID') == block_id
                for inst in gpu.get('instances', [])
            )
            if blockid_present and model_count < max_model_per_gpu:
                result.append((node, gpu))
    return result
